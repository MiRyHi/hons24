...
def calculate_prediction_percentages_thresh(TP, FP, TN, FN, inference_stats, training_object_name, AUC_value_trn, AUC_value_inf, mcc, log_loss_value, kappa, threshold, log_inference_dir):
...
def calculate_prediction_stats_thresh(df_pred, input_df, training_object_name, threshold, true_labels, predicted_probs, AUC_value_inf):
...
def create_inference_ranks(log_inference_dir, thresholds):
    """
    ...
    for thresh in thresholds:
        ...
        if os.path.exists(inference_stats_file):
            ...
            if all(col in inference_stats.columns for col in rank_columns_base):
                ...
                missing_cols = [col for col in rank_columns_base if col not in inference_stats.columns]
...
def initiate_training(training_object_name, settings, log_inference_dir):
    ...
    train_loss_min = common_settings.get('train_loss_min', 0.0)
...
def update_inference_stats(Inference_stats, training_object_name, TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, accuracy, AUC_value_trn, AUC_value_inf, mcc, log_loss_value, kappa, fall_out, miss_rate, lr_plus, lr_minus):
...
num_fc_layers_values = [1, 0]
kf_fold_values = [5]
mc_fold_values = [20]
weight_by_class_values = [False, True]
        ...
        for variation_num, settings_tuple in enumerate(combinations, start=1):
            ...
            for thresh in thresholds:
                ...
                TP, FP, TN, FN, accuracy, precision, recall, specificity, f1_score, mcc, log_loss_value, kappa, fpr, fnr, npv, lr_plus, lr_minus = calculate_prediction_stats_thresh(
                    df_pred, input_df, training_object_name, thresh, true_labels, predicted_probs, AUC_value_inf
                )

                # Update inference stats for the current threshold
                calculate_prediction_percentages_thresh(
                    TP, FP, TN, FN, inference_stats, training_object_name, 
            ...
            create_inference_ranks(log_inference_dir, thresholds)
