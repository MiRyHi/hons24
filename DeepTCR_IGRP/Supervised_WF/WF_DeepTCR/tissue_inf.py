...
def inference_bld_pd1():
   inference_data_file = f'{base_path}/WF_DeepTCR/Data_valid/Collier_2023/bld_PD1_cd8'
   inference_name = "bld_pd1"
   DTCR_WF.Get_Data(directory=inference_data_file,
                    Load_Prev_Data=False,
                    aggregate_by_aa=False,
                    sep=',',
                    ...
                    d_beta_column=3,
                    j_beta_column=4)
   print("inference_bld_pd1 data input successful")
   return inference_name
...
def inference_pln_pd1():
   inference_data_file = f'{base_path}/WF_DeepTCR/Data_valid/Collier_2023/pLN_PD1_cd8'
   inference_name = "pln_pd1"
   DTCR_WF.Get_Data(directory=inference_data_file,
                    Load_Prev_Data=False,
                    aggregate_by_aa=False,
                    sep=',',
                    ...
                    d_beta_column=3,
                    j_beta_column=4)
   print("inference_pln_pd1 data input successful")
   return inference_name
...
def inference_panc_pd1():
   inference_data_file = f'{base_path}/WF_DeepTCR/Data_valid/Collier_2023/pan_PD1_cd8'
   inference_name = "panc_pd1"
   DTCR_WF.Get_Data(directory=inference_data_file,
                    Load_Prev_Data=False,
                    aggregate_by_aa=False,
                    sep=',',
                    ...
                    d_beta_column=3,
                    j_beta_column=4)
   print("inference_panc_pd1 data input successful")
   return inference_name
...
def calculate_inf_predictions_thresh(df_infpred, training_object_name, threshold, inference_name):
    ...
    # Access the second column using integer indexing
    df_infpred[pred_column] = np.where(df_infpred[1] > threshold / 100.0, 'TETpos', 'TETneg')
    
    # Count how many TETpos values are present
...
# Get a sorted list of folders that start with "IGRP_WF_"
...
# Ask the user if they wish to proceed with inference
...
if proceed.lower() == 'true':
    ...
    # Loop through each folder and assign the training_object_name
    ...
    inf_frac_bld_pd1 = pd.DataFrame(columns=['model name', 't50_bld_pd1', 't60_bld_pd1', 't70_bld_pd1', 't80_bld_pd1', 't90_bld_pd1', 'nseq_bld_pd1'])
    ...
    inf_frac_pln_pd1 = pd.DataFrame(columns=['model name', 't50_pln_pd1', 't60_pln_pd1', 't70_pln_pd1', 't80_pln_pd1', 't90_pln_pd1', 'nseq_pln_pd1'])
    ...
    inf_frac_panc_pd1 = pd.DataFrame(columns=['model name', 't50_panc_pd1', 't60_panc_pd1', 't70_panc_pd1', 't80_panc_pd1', 't90_panc_pd1', 'nseq_panc_pd1'])
    ...
    # ---- Process inference_bld_pd1 ----
        inference_name = inference_bld_pd1()  # This returns 'bld_pd1'
        ...
        # Create a new row for this training_object_name
        new_row_pd1 = {'model name': training_object_name}
        total_seq = df_infpred.shape[0]
        new_row_pd1['nseq_bld_pd1'] = total_seq
        
        # Loop through thresholds for bld_pd1
        for thresh in thresholds:
            tetpos_fraction = calculate_inf_predictions_thresh(df_infpred, training_object_name, thresh, inference_name)
            new_row_pd1[f't{thresh}_{inference_name}'] = tetpos_fraction
        
        # Append the new row to the inf_frac_bld_pd1 DataFrame
        inf_frac_bld_pd1 = inf_frac_bld_pd1.append(new_row_pd1, ignore_index=True)
        
    # ---- Process inference_bld_spt ----
        inference_name = inference_bld_spt()  # This returns 'bld_spt'
    ...
    # ---- Process inference_pln_igg ----
        ...
        # Loop through thresholds for pln_igg
        for thresh in thresholds:
            tetpos_fraction = calculate_inf_predictions_thresh(df_infpred, training_object_name, thresh, inference_name)
            new_row_igg_pln[f't{thresh}_{inference_name}'] = tetpos_fraction
        
        # Append the new row to the inf_frac_pln_igg DataFrame
        inf_frac_pln_igg = inf_frac_pln_igg.append(new_row_igg_pln, ignore_index=True)
            
    # ---- Process inference_pln_pd1 ----
        inference_name = inference_pln_pd1()  # This returns 'pln_pd1'
        ...
        # Create a new row for this training_object_name
        new_row_pd1_pln = {'model name': training_object_name}
        total_seq = df_infpred.shape[0]
        new_row_pd1_pln['nseq_pln_pd1'] = total_seq
        
        # Loop through thresholds for pln_pd1
        for thresh in thresholds:
            tetpos_fraction = calculate_inf_predictions_thresh(df_infpred, training_object_name, thresh, inference_name)
            new_row_pd1_pln[f't{thresh}_{inference_name}'] = tetpos_fraction
        
        # Append the new row to the inf_frac_pln_pd1 DataFrame
        inf_frac_pln_pd1 = inf_frac_pln_pd1.append(new_row_pd1_pln, ignore_index=True)
        
    # ---- Process inference_pln_spt ----
        inference_name = inference_pln_spt()  # This returns 'pln_spt'
    ...
    # ---- Process inference_panc_igg ----
        ...
        # Loop through thresholds for panc_igg
        for thresh in thresholds:
            tetpos_fraction = calculate_inf_predictions_thresh(df_infpred, training_object_name, thresh, inference_name)
            new_row_igg_panc[f't{thresh}_{inference_name}'] = tetpos_fraction
        
        # Append the new row to the inf_frac_panc_igg DataFrame
        inf_frac_panc_igg = inf_frac_panc_igg.append(new_row_igg_panc, ignore_index=True)
           
    # ---- Process inference_panc_pd1 ----
        inference_name = inference_panc_pd1()  # This returns 'panc_pd1'
        ...
        # Create a new row for this training_object_name
        new_row_pd1_panc = {'model name': training_object_name}
        total_seq = df_infpred.shape[0]
        new_row_pd1_panc['nseq_panc_pd1'] = total_seq
        
        # Loop through thresholds for panc_pd1
        for thresh in thresholds:
            tetpos_fraction = calculate_inf_predictions_thresh(df_infpred, training_object_name, thresh, inference_name)
            new_row_pd1_panc[f't{thresh}_{inference_name}'] = tetpos_fraction
        
        # Append the new row to the inf_frac_panc_pd1 DataFrame
        inf_frac_panc_pd1 = inf_frac_panc_pd1.append(new_row_pd1_panc, ignore_index=True)
        
    # ---- Process inference_panc_spt ----
        inference_name = inference_panc_spt()  # This returns 'panc_spt'
        ...
        # Loop through thresholds for panc_spt
        for thresh in thresholds:
            tetpos_fraction = calculate_inf_predictions_thresh(df_infpred, training_object_name, thresh, inference_name)
            new_row_spt_panc[f't{thresh}_{inference_name}'] = tetpos_fraction
        
        # Append the new row to the inf_frac_panc_spt DataFrame
    ...
    inf_frac_bld_pd1.to_csv(f'{log_inf_stats_folder}/inf_frac_bld_pd1.csv', index=False)
    ...
    inf_frac_pln_pd1.to_csv(f'{log_inf_stats_folder}/inf_frac_pln_pd1.csv', index=False)
    ...
    inf_frac_panc_pd1.to_csv(f'{log_inf_stats_folder}/inf_frac_panc_pd1.csv', index=False)
    inf_frac_panc_spt.to_csv(f'{log_inf_stats_folder}/inf_frac_panc_spt.csv', index=False)

else:
    print("Exiting without running inference.")

dataframes = [inf_frac_bld_igg, inf_frac_bld_pd1, inf_frac_bld_spt, inf_frac_pln_igg, inf_frac_pln_pd1, inf_frac_pln_spt, inf_frac_panc_igg, inf_frac_panc_pd1, inf_frac_panc_spt]
...
metrics_overview_50 = pd.merge(metrics_overview_50, inf_frac_bld_pd1[['model name', 't50_bld_pd1', 'nseq_bld_pd1']], on='model name', how='left')
...
metrics_overview_50 = pd.merge(metrics_overview_50, inf_frac_pln_pd1[['model name', 't50_pln_pd1', 'nseq_pln_pd1']], on='model name', how='left')
...
metrics_overview_50 = pd.merge(metrics_overview_50, inf_frac_panc_pd1[['model name', 't50_panc_pd1', 'nseq_panc_pd1']], on='model name', how='left')
...
metrics_overview_60 = pd.merge(metrics_overview_60, inf_frac_bld_pd1[['model name', 't60_bld_pd1', 'nseq_bld_pd1']], on='model name', how='left')
...
metrics_overview_60 = pd.merge(metrics_overview_60, inf_frac_pln_pd1[['model name', 't60_pln_pd1', 'nseq_pln_pd1']], on='model name', how='left')
...
metrics_overview_60 = pd.merge(metrics_overview_60, inf_frac_panc_pd1[['model name', 't60_panc_pd1', 'nseq_panc_pd1']], on='model name', how='left')
...
metrics_overview_70 = pd.merge(metrics_overview_70, inf_frac_bld_pd1[['model name', 't70_bld_pd1', 'nseq_bld_pd1']], on='model name', how='left')
...
metrics_overview_70 = pd.merge(metrics_overview_70, inf_frac_pln_pd1[['model name', 't70_pln_pd1', 'nseq_pln_pd1']], on='model name', how='left')
...
metrics_overview_70 = pd.merge(metrics_overview_70, inf_frac_panc_pd1[['model name', 't70_panc_pd1', 'nseq_panc_pd1']], on='model name', how='left')
...
metrics_overview_80 = pd.merge(metrics_overview_80, inf_frac_bld_pd1[['model name', 't80_bld_pd1', 'nseq_bld_pd1']], on='model name', how='left')
...
metrics_overview_80 = pd.merge(metrics_overview_80, inf_frac_pln_pd1[['model name', 't80_pln_pd1', 'nseq_pln_pd1']], on='model name', how='left')
...
metrics_overview_80 = pd.merge(metrics_overview_80, inf_frac_panc_pd1[['model name', 't80_panc_pd1', 'nseq_panc_pd1']], on='model name', how='left')
...
metrics_overview_90 = pd.merge(metrics_overview_90, inf_frac_bld_pd1[['model name', 't90_bld_pd1', 'nseq_bld_pd1']], on='model name', how='left')
...
metrics_overview_90 = pd.merge(metrics_overview_90, inf_frac_pln_pd1[['model name', 't90_pln_pd1', 'nseq_pln_pd1']], on='model name', how='left')
...
metrics_overview_90 = pd.merge(metrics_overview_90, inf_frac_panc_pd1[['model name', 't90_panc_pd1', 'nseq_panc_pd1']], on='model name', how='left')
