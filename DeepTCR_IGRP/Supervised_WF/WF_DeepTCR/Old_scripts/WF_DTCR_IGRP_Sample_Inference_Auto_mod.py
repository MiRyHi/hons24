...
def checkpoint_after_group(counter, training_type, common_settings, log_inference_dir):
    if counter > 0 and counter % 25 == 0:
        ...
        checkpoint_file = os.path.join(checkpoint_dir, f"{training_type}_checkpoint_group_{counter // 25}.pkl")
        save_checkpoint(checkpoint_data, checkpoint_file)
        print(f"Checkpoint saved for {training_type} after {counter} models.")
...
def calculate_prediction_percentages_thresh(TP, FP, TN, FN, inference_stats, training_object_name, inference_data_name, AUC_value_trn, AUC_value_inf, mcc, log_loss_value, kappa, threshold, log_inference_dir):
    ...
    required_columns = [
        'model_name',
        f'TP ({inference_data_name})', f'FP ({inference_data_name})', f'TN ({inference_data_name})', f'FN ({inference_data_name})',
        f'accuracy ({inference_data_name})', f'precision ({inference_data_name})',
        f'recall ({inference_data_name})', f'specificity ({inference_data_name})', f'f1_score ({inference_data_name})',
        f'b_accuracy ({inference_data_name})', f'AUC_value_inf ({inference_data_name})', f'AUC_value_trn ({inference_data_name})',
        f'mcc ({inference_data_name})', f'log_loss ({inference_data_name})', f'kappa ({inference_data_name})',
        f'fall_out ({inference_data_name})', f'miss_rate ({inference_data_name})', f'NPV ({inference_data_name})',
        f'lr_plus ({inference_data_name})', f'lr_minus ({inference_data_name})'
    ]
    ...
    metrics = {
        f'TP ({inference_data_name})': TP, 
        f'FP ({inference_data_name})': FP, 
        f'TN ({inference_data_name})': TN, 
        f'FN ({inference_data_name})': FN,
        f'accuracy ({inference_data_name})': accuracy,
        f'precision ({inference_data_name})': precision,
        f'recall ({inference_data_name})': recall,
        f'specificity ({inference_data_name})': specificity,
        f'f1_score ({inference_data_name})': f1_score,
        f'b_accuracy ({inference_data_name})': b_accuracy,
        f'AUC_value_inf ({inference_data_name})': AUC_value_inf,
        f'AUC_value_trn ({inference_data_name})': AUC_value_trn,
        f'mcc ({inference_data_name})': mcc, 
        f'log_loss ({inference_data_name})': log_loss_value, 
        f'kappa ({inference_data_name})': kappa,
        f'fall_out ({inference_data_name})': fall_out,
        f'miss_rate ({inference_data_name})': miss_rate,
        f'NPV ({inference_data_name})': NPV,
        f'lr_plus ({inference_data_name})': lr_plus,
        f'lr_minus ({inference_data_name})': lr_minus
    }
...
def create_inference_ranks(log_inference_dir, thresholds, inference_data_name):
    ...
    """
    # List of columns to rank, dynamically adjusting to include inference_data_name in brackets
    rank_columns_base = ['TP', 'FP', 'TN', 'FN', 'accuracy', 'precision', 'recall', 'specificity', 
                         'f1_score', 'b_accuracy', 'AUC_value_inf', 'AUC_value_trn', 'mcc', 'log_loss', 'kappa', 
                         'fall_out', 'miss_rate', 'NPV', 'lr_plus', 'lr_minus']
...
def create_inference_stats_55(log_inference_dir):
    global Inference_stats_55
    inference_stats_file_55 = os.path.join(log_inference_dir, 'Inference_stats_55.csv').replace("\\", "/")
    
    if os.path.exists(inference_stats_file_55):
        Inference_stats_55 = pd.read_csv(inference_stats_file_55)
    else:
        Inference_stats_55 = pd.DataFrame(columns=['model_name'])
        Inference_stats_55.to_csv(inference_stats_file_55, index=False)  # Save after initialization
...
def create_inference_stats_85(log_inference_dir):
    global Inference_stats_85
    inference_stats_file_85 = os.path.join(log_inference_dir, 'Inference_stats_85.csv').replace("\\", "/")
    ...
    if os.path.exists(inference_stats_file_85):
        ...
        Inference_stats_85.to_csv(inference_stats_file_85, index=False)  # Save after initialization
...
def create_log_inference_directory(training_data_name):
    log_inference_dir = os.path.join(base_path, f'log_inference_{training_data_name}/').replace("\\", "/")
    if not os.path.exists(log_inference_dir):
        os.makedirs(log_inference_dir)
...
def create_metrics_overview(threshold, log_inference_dir):
    ...
    merged_file_path = os.path.join(log_inference_dir, f'Metrics_overview_{threshold}.csv').replace("\\", "/")
...
def initiate_training(training_object_name, settings, log_inference_dir):
    ...
    train_loss_min = common_settings.get('train_loss_min', 0.1)
    ...
    combine_train_valid = common_settings.get('combine_train_valid', True)
    ...
    # Handle the specific training type
    if 'Train' in training_object_name:
        ...
        DTCR_WF.Get_Train_Valid_Test(test_size=test_size, LOO=LOO,
        ...
        # AUC curve and logging after training
        DTCR_WF.AUC_Curve()
        AUC_value_trn = DTCR_WF.AUC_DF['AUC'].mean()

        # Log training details for Train model
        log_training_details(training_object_name, training_type, log_inference_dir, **common_settings)

    elif 'Kf' in training_object_name:
        print(f"Training model: {training_object_name}")
        training_type = 'K-fold'
        suppress_output = False
        folds = common_settings['folds']
        weight_by_class = common_settings['weight_by_class']
        kernel = common_settings['kernel']
        num_concepts = common_settings['num_concepts']
        size_of_net = common_settings['size_of_net']
        ...
        # Log training details only once after processing
        log_training_details(training_object_name, training_type, log_inference_dir, **common_settings)
...
def log_training_details(training_object_name, training_type, log_inference_dir, **kwargs):
    # Determine the correct values for LOO and Folds
    if training_type == 'K-fold':
        loo_value = 'N.A.'  # LOO is N.A. for K-fold
        test_size_value = 'N.A.'
        folds_value = kwargs.get('folds', 'N.A.')  # Folds value for K-fold
    elif training_type == 'Monte Carlo':
        loo_value = kwargs.get('LOO', 'None')  # For Monte Carlo, use the actual LOO value
        folds_value = kwargs.get('folds', 'N.A.')  # Folds value for Monte Carlo
        test_size_value = kwargs.get('test_size', 'N.A.')
    else:
        loo_value = kwargs.get('LOO', 'None')  # For Train, use the actual LOO value if provided
        test_size_value = kwargs.get('test_size', 'N.A.')
        folds_value = 'N.A.'  # Folds not applicable for Train
    ...
    training_details = {
        ...
        'train_loss_min': kwargs.get('train_loss_min', 'N.A.'),  # Specific train_loss_min value
...
def update_inference_stats(Inference_stats, training_object_name, inference_data_name, TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, accuracy, AUC_value_trn, AUC_value_inf, mcc, log_loss_value, kappa, fall_out, miss_rate, lr_plus, lr_minus):
    ...
    # Create dynamic column names that include inference_data_name
    ...
    col_accuracy = f'accuracy ({inference_data_name})'
    col_b_accuracy = f'b_accuracy ({inference_data_name})'
    ...
    col_ROC_trn = f'ROC Trn ({inference_data_name})'
    col_ROC_inf = f'ROC Inf ({inference_data_name})'
    col_FPR = f'FPR ({inference_data_name})'
    col_FNR = f'FNR ({inference_data_name})'
    col_MCC = f'MCC ({inference_data_name})'
    col_Log_Loss = f'Log Loss ({inference_data_name})'
    col_Kappa = f'Kappa ({inference_data_name})'
    col_fall_out = f'fall_out ({inference_data_name})'
    col_miss_rate = f'miss_rate ({inference_data_name})'
    col_lr_plus = f'lr_plus ({inference_data_name})'
    col_lr_minus = f'lr_minus ({inference_data_name})'
...
# Variable settings (used to calculate different setting combinations)
test_size_values = [0.20, 0.25]
LOO_values = [2]
size_of_net_values = ['small', 'medium']
kernel_values = [3, 5, 7, 9]
num_concepts_values = [12, 24]
num_agg_layers_values = [0]
num_fc_layers_values = [0, 1]
folds_values = [5]
weight_by_class_values = [True, False] 

# Initialise empty list to hold all setting combinations
common_settings_variations = []

# Generate Train settings (includes LOO, no folds)
for size_of_net, kernel, num_concepts, LOO, test_size, num_fc_layers, num_agg_layers, weight_by_class in itertools.product(
    size_of_net_values, kernel_values, num_concepts_values, LOO_values, test_size_values, num_fc_layers_values, num_agg_layers_values, weight_by_class_values):
    
    train_settings = deepcopy(base_common_settings)
    train_settings['test_size'] = test_size
    train_settings['LOO'] = LOO
    train_settings['size_of_net'] = size_of_net
    train_settings['kernel'] = kernel
    train_settings['num_concepts'] = num_concepts
    train_settings['num_fc_layers'] = num_fc_layers
    train_settings['num_agg_layers'] = num_agg_layers
    train_settings['weight_by_class'] = weight_by_class
    common_settings_variations.append(('Train', train_settings))

# Generate K-fold settings (uses folds, no LOO)
for size_of_net, kernel, num_concepts, folds, num_fc_layers, num_agg_layers, weight_by_class in itertools.product(
    size_of_net_values, kernel_values, num_concepts_values, folds_values, num_fc_layers_values, num_agg_layers_values, weight_by_class_values):
    
    kf_settings = deepcopy(base_common_settings)
    kf_settings['folds'] = folds
...
# Generate Monte Carlo settings (using both LOO and folds)
for size_of_net, kernel, num_concepts, folds, LOO, test_size, num_fc_layers, num_agg_layers, weight_by_class in itertools.product(
    size_of_net_values, kernel_values, num_concepts_values, folds_values, LOO_values, test_size_values, num_fc_layers_values, num_agg_layers_values, weight_by_class_values):
    
    mc_settings = deepcopy(base_common_settings)
    mc_settings['test_size'] = test_size
    mc_settings['LOO'] = LOO
    mc_settings['folds'] = folds
...
triplicate_training = ['Train', 'Kf', 'Mc']
training_data_name = 'Tp1-8_Tn1-8_0_cut'
...
# Reset counts
train_count, kf_count, mc_count = 0, 0, 0

# Iterate over all common settings variations
for settings in common_settings_variations:
    model_type, common_settings = settings

    # Train Models - only use LOO, no folds
    if model_type == 'Train':
        if 'folds' in common_settings:
            print(f"Skipping Train model due to 'folds': {settings}")
            continue  # Skip any Train models that have 'folds' set
        if 'LOO' not in common_settings or 'test_size' not in common_settings:
            print(f"Skipping Train model due to missing 'LOO' or 'test_size': {settings}")
            continue  # Skip Train models without 'LOO' or 'test_size'
        train_combinations.append(settings)
        train_count += 1
...
train_combinations.sort(key=lambda x: (
    x[1]['weight_by_class'],
    x[1]['test_size'],
    x[1]['LOO'],  # LOO is always present in Train settings
    x[1]['kernel'],
    x[1]['size_of_net'],
    x[1]['num_concepts'],
    x[1]['num_fc_layers'],
...
total_train_models = len(train_combinations)
...
print(f"\nTotal Train Models: {total_train_models}")
...
if start_training == 'true':
    ...
    for model_type in ['Train', 'Kf', 'Mc']:
        ...
        for variation_num, settings_tuple in enumerate(combinations, start=1):
            ...
            if model_type == 'Mc':
                # For Monte Carlo, vary both folds and LOO, no need for changes
                pass
            ...
            # Corrected: Pass the full tuple (model_type, common_settings)
            ...
            for thresh in thresholds:
                ...
                if thresh == 50:
                    ...
                    create_inference_stats_55(log_inference_dir)
                elif thresh == 70:
                    create_inference_stats_70(log_inference_dir)
                elif thresh == 85:
                    create_inference_stats_85(log_inference_dir)
                elif thresh == 90:
        ...
        # Always save a final checkpoint to capture remaining models or the last models.
        print(f"Saving final checkpoint for {model_type} after all models have been trained.")
