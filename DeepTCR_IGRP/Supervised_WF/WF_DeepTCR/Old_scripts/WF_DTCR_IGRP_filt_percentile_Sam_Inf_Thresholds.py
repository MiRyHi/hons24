...
def calculate_prediction_stats_thresh_65(df_pred, input_df, training_object_name, inference_data_name):
    # Check if df_pred and input_df have the same number of rows
    if df_pred.shape[0] == input_df.shape[0]:
        print("Both df_pred and input_df have the same number of rows.")
    else:
    ...
    # Use the 0.65 threshold for 'Pred' column based only on 'TETpos_prob'
    df_pred['Pred'] = np.where(df_pred['TETpos_prob'] > 0.65, 'TETpos', 'TETneg')
    ...
    # Return the TP, FP, TN, FN values and pass them to update_inference_stats_65
    return TP, FP, TN, FN, P, N

def calculate_prediction_stats_thresh_75(df_pred, input_df, training_object_name, inference_data_name):
    # Check if df_pred and input_df have the same number of rows
    if df_pred.shape[0] == input_df.shape[0]:
        print("Both df_pred and input_df have the same number of rows.")
    else:
    ...
    # Use the 0.75 threshold for 'Pred' column based only on 'TETpos_prob'
    df_pred['Pred'] = np.where(df_pred['TETpos_prob'] > 0.75, 'TETpos', 'TETneg')
    ...
    # Return the TP, FP, TN, FN values and pass them to update_inference_stats_75
    return TP, FP, TN, FN, P, N

def calculate_prediction_stats_thresh_85(df_pred, input_df, training_object_name, inference_data_name):
    # Check if df_pred and input_df have the same number of rows
    if df_pred.shape[0] == input_df.shape[0]:
        print("Both df_pred and input_df have the same number of rows.")
    else:
    ...
    # Use the 0.85 threshold for 'Pred' column based only on 'TETpos_prob'
    df_pred['Pred'] = np.where(df_pred['TETpos_prob'] > 0.85, 'TETpos', 'TETneg')
    ...
    # Return the TP, FP, TN, FN values and pass them to update_inference_stats_85
    return TP, FP, TN, FN, P, N

def calculate_prediction_stats_thresh_95(df_pred, input_df, training_object_name, inference_data_name):
    # Check if df_pred and input_df have the same number of rows
    if df_pred.shape[0] == input_df.shape[0]:
        print("Both df_pred and input_df have the same number of rows.")
    else:
    ...
    # Use the 0.95 threshold for 'Pred' column based only on 'TETpos_prob'
    df_pred['Pred'] = np.where(df_pred['TETpos_prob'] > 0.95, 'TETpos', 'TETneg')
    ...
    # Return the TP, FP, TN, FN values and pass them to update_inference_stats_95
    return TP, FP, TN, FN, P, N
...
def create_inference_stats_65():
    global Inference_stats_65  # Use global variable for the 65 threshold version
    inference_stats_file_65 = '/Users/Mitch/Documents/DeepTCR_IGRP/Supervised_WF/Inference_stats_65.csv'
    
    if os.path.exists(inference_stats_file_65):
        Inference_stats_65 = pd.read_csv(inference_stats_file_65)
    else:
        Inference_stats_65 = pd.DataFrame(columns=['model_name'])
        
def create_inference_stats_75():
    global Inference_stats_75  # Use global variable for the 75 threshold version
    inference_stats_file_75 = '/Users/Mitch/Documents/DeepTCR_IGRP/Supervised_WF/Inference_stats_75.csv'
    
    if os.path.exists(inference_stats_file_75):
        Inference_stats_75 = pd.read_csv(inference_stats_file_75)
    else:
        Inference_stats_75 = pd.DataFrame(columns=['model_name'])
        
def create_inference_stats_85():
    global Inference_stats_85  # Use global variable for the 85 threshold version
    inference_stats_file_85 = '/Users/Mitch/Documents/DeepTCR_IGRP/Supervised_WF/Inference_stats_85.csv'
    
    if os.path.exists(inference_stats_file_85):
        Inference_stats_85 = pd.read_csv(inference_stats_file_85)
    else:
        Inference_stats_85 = pd.DataFrame(columns=['model_name'])

def create_inference_stats_95():
    global Inference_stats_95  # Use global variable for the 95 threshold version
    inference_stats_file_95 = '/Users/Mitch/Documents/DeepTCR_IGRP/Supervised_WF/Inference_stats_95.csv'
    
    if os.path.exists(inference_stats_file_95):
        Inference_stats_95 = pd.read_csv(inference_stats_file_95)
    else:
        Inference_stats_95 = pd.DataFrame(columns=['model_name'])
...
def update_inference_stats_65(Inference_stats_65, training_object_name, inference_data_name, TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, AUC_value):
    # Ensure no duplicate columns exist
    Inference_stats_65 = Inference_stats_65.loc[:, ~Inference_stats_65.columns.duplicated()]
    ...
    # Check if the model_name already exists in the DataFrame
    if 'model_name' in Inference_stats_65.columns:
        matching_indices = Inference_stats_65.index[Inference_stats_65['model_name'] == training_object_name]

        if len(matching_indices) > 0:
            index = matching_indices[0]
            
            for col in [col_TP, col_FP, col_TN, col_FN, col_b_accuracy, col_PPV, col_NPV, col_TPR, col_TNR, col_F1, col_ROC, col_FPR, col_FNR]:
                if col not in Inference_stats_65.columns:
                    Inference_stats_65[col] = np.nan

            Inference_stats_65.at[index, col_TP] = TP
            Inference_stats_65.at[index, col_FP] = FP
            Inference_stats_65.at[index, col_TN] = TN
            Inference_stats_65.at[index, col_FN] = FN
            Inference_stats_65.at[index, col_b_accuracy] = b_accuracy
            Inference_stats_65.at[index, col_PPV] = PPV
            Inference_stats_65.at[index, col_NPV] = NPV
            Inference_stats_65.at[index, col_TPR] = TPR
            Inference_stats_65.at[index, col_TNR] = TNR
            Inference_stats_65.at[index, col_F1] = F1
            Inference_stats_65.at[index, col_ROC] = AUC_value
            Inference_stats_65.at[index, col_FPR] = FPR
            Inference_stats_65.at[index, col_FNR] = FNR
        else:
            new_row = {
                'model_name': training_object_name,
                col_TP: TP,
            ...
            Inference_stats_65 = Inference_stats_65.append(new_row, ignore_index=True)
    else:
        new_row = {
            'model_name': training_object_name,
            col_TP: TP,
        ...
        Inference_stats_65 = pd.DataFrame([new_row])

    Inference_stats_65 = Inference_stats_65.loc[:, ~Inference_stats_65.columns.duplicated()]
    all_inference_data_names = [col.split('(')[1].split(')')[0] for col in Inference_stats_65.columns if '(' in col]
    unique_inference_data_names = sorted(set(all_inference_data_names))  # Get unique inference_data_name
    ...
    Inference_stats_65 = Inference_stats_65[[col for col in ordered_columns if col in Inference_stats_65.columns]]
    return Inference_stats_65

def update_inference_stats_75(Inference_stats_75, training_object_name, inference_data_name, TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, AUC_value):
    # Ensure no duplicate columns exist
    Inference_stats_75 = Inference_stats_75.loc[:, ~Inference_stats_75.columns.duplicated()]
    ...
    # Check if the model_name already exists in the DataFrame
    if 'model_name' in Inference_stats_75.columns:
        matching_indices = Inference_stats_75.index[Inference_stats_75['model_name'] == training_object_name]

        if len(matching_indices) > 0:
            index = matching_indices[0]
            
            for col in [col_TP, col_FP, col_TN, col_FN, col_b_accuracy, col_PPV, col_NPV, col_TPR, col_TNR, col_F1, col_ROC, col_FPR, col_FNR]:
                if col not in Inference_stats_75.columns:
                    Inference_stats_75[col] = np.nan

            Inference_stats_75.at[index, col_TP] = TP
            Inference_stats_75.at[index, col_FP] = FP
            Inference_stats_75.at[index, col_TN] = TN
            Inference_stats_75.at[index, col_FN] = FN
            Inference_stats_75.at[index, col_b_accuracy] = b_accuracy
            Inference_stats_75.at[index, col_PPV] = PPV
            Inference_stats_75.at[index, col_NPV] = NPV
            Inference_stats_75.at[index, col_TPR] = TPR
            Inference_stats_75.at[index, col_TNR] = TNR
            Inference_stats_75.at[index, col_F1] = F1
            Inference_stats_75.at[index, col_ROC] = AUC_value
            Inference_stats_75.at[index, col_FPR] = FPR
            Inference_stats_75.at[index, col_FNR] = FNR
        else:
            new_row = {
                'model_name': training_object_name,
                col_TP: TP,
            ...
            Inference_stats_75 = Inference_stats_75.append(new_row, ignore_index=True)
    else:
        new_row = {
            'model_name': training_object_name,
            col_TP: TP,
        ...
        Inference_stats_75 = pd.DataFrame([new_row])

    Inference_stats_75 = Inference_stats_75.loc[:, ~Inference_stats_75.columns.duplicated()]
    all_inference_data_names = [col.split('(')[1].split(')')[0] for col in Inference_stats_75.columns if '(' in col]
    unique_inference_data_names = sorted(set(all_inference_data_names))  # Get unique inference_data_name
    ...
    Inference_stats_75 = Inference_stats_75[[col for col in ordered_columns if col in Inference_stats_75.columns]]
    return Inference_stats_75

def update_inference_stats_85(Inference_stats_85, training_object_name, inference_data_name, TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, AUC_value):
    # Ensure no duplicate columns exist
    Inference_stats_85 = Inference_stats_85.loc[:, ~Inference_stats_85.columns.duplicated()]
    ...
    # Check if the model_name already exists in the DataFrame
    if 'model_name' in Inference_stats_85.columns:
        matching_indices = Inference_stats_85.index[Inference_stats_85['model_name'] == training_object_name]
        ...
        if len(matching_indices) > 0:
            ...
            # Add columns if they don't exist for the specific dataset (e.g., Kas_25_cut)
            for col in [col_TP, col_FP, col_TN, col_FN, col_b_accuracy, col_PPV, col_NPV, col_TPR, col_TNR, col_F1, col_ROC, col_FPR, col_FNR]:
                if col not in Inference_stats_85.columns:
                    Inference_stats_85[col] = np.nan

            # Now update the values for the current inference_data_name
            Inference_stats_85.at[index, col_TP] = TP
            Inference_stats_85.at[index, col_FP] = FP
            Inference_stats_85.at[index, col_TN] = TN
            Inference_stats_85.at[index, col_FN] = FN
            Inference_stats_85.at[index, col_b_accuracy] = b_accuracy
            Inference_stats_85.at[index, col_PPV] = PPV
            Inference_stats_85.at[index, col_NPV] = NPV
            Inference_stats_85.at[index, col_TPR] = TPR
            Inference_stats_85.at[index, col_TNR] = TNR
            Inference_stats_85.at[index, col_F1] = F1
            Inference_stats_85.at[index, col_ROC] = AUC_value
            Inference_stats_85.at[index, col_FPR] = FPR
            Inference_stats_85.at[index, col_FNR] = FNR
        else:
            # Add a new row if no matching model_name is found, with the new dataset's statistics
            new_row = {
                'model_name': training_object_name,
            ...
            Inference_stats_85 = Inference_stats_85.append(new_row, ignore_index=True)
    else:
        # If the DataFrame is empty or missing columns, initialize with a new row
        new_row = {
            'model_name': training_object_name,
        ...
        Inference_stats_85 = pd.DataFrame([new_row])

    # Ensure no duplicate columns
    Inference_stats_85 = Inference_stats_85.loc[:, ~Inference_stats_85.columns.duplicated()]

    # Enforce the correct column order for all datasets in the DataFrame
    all_inference_data_names = [col.split('(')[1].split(')')[0] for col in Inference_stats_85.columns if '(' in col]
    unique_inference_data_names = sorted(set(all_inference_data_names))  # Get unique inference_data_name
    ...
    # Reorder DataFrame to follow the correct column order
    Inference_stats_85 = Inference_stats_85[[col for col in ordered_columns if col in Inference_stats_85.columns]]

    return Inference_stats_85

def update_inference_stats_95(Inference_stats_95, training_object_name, inference_data_name, TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, AUC_value):
    # Ensure no duplicate columns exist
    Inference_stats_95 = Inference_stats_95.loc[:, ~Inference_stats_95.columns.duplicated()]
    ...
    # Check if the model_name already exists in the DataFrame
    if 'model_name' in Inference_stats_95.columns:
        matching_indices = Inference_stats_95.index[Inference_stats_95['model_name'] == training_object_name]

        if len(matching_indices) > 0:
            index = matching_indices[0]
            
            for col in [col_TP, col_FP, col_TN, col_FN, col_b_accuracy, col_PPV, col_NPV, col_TPR, col_TNR, col_F1, col_ROC, col_FPR, col_FNR]:
                if col not in Inference_stats_95.columns:
                    Inference_stats_95[col] = np.nan

            Inference_stats_95.at[index, col_TP] = TP
            Inference_stats_95.at[index, col_FP] = FP
            Inference_stats_95.at[index, col_TN] = TN
            Inference_stats_95.at[index, col_FN] = FN
            Inference_stats_95.at[index, col_b_accuracy] = b_accuracy
            Inference_stats_95.at[index, col_PPV] = PPV
            Inference_stats_95.at[index, col_NPV] = NPV
            Inference_stats_95.at[index, col_TPR] = TPR
            Inference_stats_95.at[index, col_TNR] = TNR
            Inference_stats_95.at[index, col_F1] = F1
            Inference_stats_95.at[index, col_ROC] = AUC_value
            Inference_stats_95.at[index, col_FPR] = FPR
            Inference_stats_95.at[index, col_FNR] = FNR
        else:
            new_row = {
                'model_name': training_object_name,
                col_TP: TP,
            ...
            Inference_stats_95 = Inference_stats_95.append(new_row, ignore_index=True)
    else:
        new_row = {
            'model_name': training_object_name,
            col_TP: TP,
        ...
        Inference_stats_95 = pd.DataFrame([new_row])

    Inference_stats_95 = Inference_stats_95.loc[:, ~Inference_stats_95.columns.duplicated()]
    all_inference_data_names = [col.split('(')[1].split(')')[0] for col in Inference_stats_95.columns if '(' in col]
    unique_inference_data_names = sorted(set(all_inference_data_names))  # Get unique inference_data_name
    ...
    Inference_stats_95 = Inference_stats_95[[col for col in ordered_columns if col in Inference_stats_95.columns]]
    return Inference_stats_95
...
def calculate_prediction_percentages_65(TP, FP, TN, FN, P, N, Inference_stats_65, training_object_name, inference_data_name, AUC_value):
    ...
    b_accuracy = 0.5 * (TPR + TNR) if P > 0 and N > 0 else 0
    ...
    Inference_stats_65 = update_inference_stats_65(
        Inference_stats_65, training_object_name, inference_data_name,
        TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, AUC_value
    )

    Inference_stats_65.to_csv(inference_stats_file_65, index=False)
    print("Finished sample inference with 0.65 threshold, calculations, and CSV writing.")

def calculate_prediction_percentages_75(TP, FP, TN, FN, P, N, Inference_stats_75, training_object_name, inference_data_name, AUC_value):
    # Calculate additional metrics (Balanced Accuracy, F1, etc.)
    PPV = TP / (TP + FP) if (TP + FP) > 0 else 0  # Positive Predictive Value
    NPV = TN / (TN + FN) if (TN + FN) > 0 else 0  # Negative Predictive Value
    TPR = TP / P if P > 0 else 0  # True Positive Rate (Recall)
    ...
    # Define the file path for inference stats CSV for 0.75 threshold
    inference_stats_file_75 = '/Users/Mitch/Documents/DeepTCR_IGRP/Supervised_WF/Inference_stats_75.csv'
    ...
    # Update the Inference_stats_75 DataFrame with the new values, including TP, FP, TN, FN
    Inference_stats_75 = update_inference_stats_75(
        Inference_stats_75, training_object_name, inference_data_name,
        TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, AUC_value
    )

    # Save the updated Inference_stats_75 DataFrame, ensuring it appends new columns if necessary
    Inference_stats_75.to_csv(inference_stats_file_75, index=False)

    print("Finished sample inference with 0.75 threshold, calculations, and CSV writing.")

def calculate_prediction_percentages_85(TP, FP, TN, FN, P, N, Inference_stats_85, training_object_name, inference_data_name, AUC_value):
    # Calculate additional metrics (Balanced Accuracy, F1, etc.)
    PPV = TP / (TP + FP) if (TP + FP) > 0 else 0  # Positive Predictive Value
    NPV = TN / (TN + FN) if (TN + FN) > 0 else 0  # Negative Predictive Value
    TPR = TP / P if P > 0 else 0  # True Positive Rate (Recall)
    ...
    # Define the file path for inference stats CSV for 0.85 threshold
    inference_stats_file_85 = '/Users/Mitch/Documents/DeepTCR_IGRP/Supervised_WF/Inference_stats_85.csv'
    ...
    # Update the Inference_stats_85 DataFrame with the new values, including TP, FP, TN, FN
    Inference_stats_85 = update_inference_stats_85(
        Inference_stats_85, training_object_name, inference_data_name,
        TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, AUC_value
    )

    # Save the updated Inference_stats_85 DataFrame, ensuring it appends new columns if necessary
    Inference_stats_85.to_csv(inference_stats_file_85, index=False)

    print("Finished sample inference with 0.85 threshold, calculations, and CSV writing.")

def calculate_prediction_percentages_95(TP, FP, TN, FN, P, N, Inference_stats_95, training_object_name, inference_data_name, AUC_value):
    # Calculate additional metrics (Balanced Accuracy, F1, etc.)
    PPV = TP / (TP + FP) if (TP + FP) > 0 else 0  # Positive Predictive Value
    NPV = TN / (TN + FN) if (TN + FN) > 0 else 0  # Negative Predictive Value
    TPR = TP / P if P > 0 else 0  # True Positive Rate (Recall)
    ...
    # Define the file path for inference stats CSV for 0.95 threshold
    inference_stats_file_95 = '/Users/Mitch/Documents/DeepTCR_IGRP/Supervised_WF/Inference_stats_95.csv'
    ...
    # Update the Inference_stats_95 DataFrame with the new values, including TP, FP, TN, FN
    Inference_stats_95 = update_inference_stats_95(
        Inference_stats_95, training_object_name, inference_data_name,
        TP, FP, TN, FN, TPR, FPR, TNR, FNR, PPV, NPV, b_accuracy, F1, AUC_value
    )

    # Save the updated Inference_stats_95 DataFrame, ensuring it appends new columns if necessary
    Inference_stats_95.to_csv(inference_stats_file_95, index=False)

    print("Finished sample inference with 0.95 threshold, calculations, and CSV writing.")
...
def initiate_training():
    ...
    if 'Train' in training_object_name:
        ...
        DTCR_WF.Get_Train_Valid_Test(test_size=test_size, LOO=LOO, combine_train_valid=combine_train_valid)
        DTCR_WF.Train(epochs_min=epochs_min, size_of_net=size_of_net, kernel=kernel, num_concepts=num_concepts,
                          train_loss_min=train_loss_min, hinge_loss_t=hinge_loss_t, qualitative_agg=qualitative_agg,
                          quantitative_agg=quantitative_agg, num_agg_layers=num_agg_layers, units_agg=units_agg,
                          num_fc_layers=num_fc_layers, units_fc=units_fc, weight_by_class=weight_by_class, 
                          multisample_dropout=multisample_dropout, multisample_dropout_rate=multisample_dropout_rate, l2_reg=l2_reg)
        DTCR_WF.AUC_Curve()
        
        AUC_value = get_auc_value(DTCR_WF)
        
        # Log training details to CSV using actual values
        log_training_details(training_object_name, input_data_folder_name, training_type, test_size=test_size, LOO=LOO, folds=None,
                             combine_train_valid=combine_train_valid, epochs_min=epochs_min, size_of_net=size_of_net, kernel=kernel, num_concepts=num_concepts,
                             train_loss_min=train_loss_min, hinge_loss_t=hinge_loss_t, qualitative_agg=qualitative_agg,
                             quantitative_agg=quantitative_agg, num_agg_layers=num_agg_layers, units_agg=units_agg,
                             num_fc_layers=num_fc_layers, units_fc=units_fc, weight_by_class=weight_by_class, 
                             multisample_dropout=multisample_dropout, multisample_dropout_rate=multisample_dropout_rate, l2_reg=l2_reg)
        ...
        DTCR_WF.K_Fold_CrossVal(combine_train_valid=combine_train_valid, hinge_loss_t=hinge_loss_t, 
                                ...
                                folds=folds, epochs_min=epochs_min, num_concepts=num_concepts, 
...
training_object_name = 'IGRP_WF_1_Train_Tp1-8_Tn1-8_0_cut'        #change to name of folder within 'Supervised_WF' that you have set up with this name format, where 'Train', 'Kf', 'Mc' are codes for the type of training, followed by _ and then the name of the training data folder (without IGRP_ at the beginning)
create_inference_stats()
create_inference_stats_65()
create_inference_stats_75()
create_inference_stats_85()
create_inference_stats_95()
...
TP, FP, TN, FN, P, N = calculate_prediction_stats_thresh_65(df_pred, input_df, training_object_name, inference_data_name)
calculate_prediction_percentages_65(TP, FP, TN, FN, P, N, Inference_stats_65, training_object_name, inference_data_name, AUC_value)
TP, FP, TN, FN, P, N = calculate_prediction_stats_thresh_75(df_pred, input_df, training_object_name, inference_data_name)
calculate_prediction_percentages_75(TP, FP, TN, FN, P, N, Inference_stats_75, training_object_name, inference_data_name, AUC_value)
TP, FP, TN, FN, P, N = calculate_prediction_stats_thresh_85(df_pred, input_df, training_object_name, inference_data_name)
calculate_prediction_percentages_85(TP, FP, TN, FN, P, N, Inference_stats_85, training_object_name, inference_data_name, AUC_value)
TP, FP, TN, FN, P, N = calculate_prediction_stats_thresh_95(df_pred, input_df, training_object_name, inference_data_name)
calculate_prediction_percentages_95(TP, FP, TN, FN, P, N, Inference_stats_95, training_object_name, inference_data_name, AUC_value)
